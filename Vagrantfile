...
PROJECT_SOURCE_URL              = "https://github.com/milewgit/layout.git"
PROJECT_VM_PATH                 = "/Users/vagrant/Documents/layout"
...
class VagrantHelper
    ...
    def install_gecode
      say "Installing gecode"
      make_tar_gz "http://www.gecode.org/download/gecode-2.2.0.tar.gz"
    end
  ...
  private
    ...
    def make_tar_gz(url)
      cache_dir = derive_cache_dir(url)
      download(url, cache_dir, "install.tar.gz")
      run_make_tar_gz(cache_dir)
    end
    ...
    def run_make_tar_gz(cache_dir)
      run_script <<-"EOF"
        mkdir install_temp
        cd install_temp
        tar -xzf #{cache_dir[:guest_path]}/install.tar.gz
        cd gecode-2.2.0
